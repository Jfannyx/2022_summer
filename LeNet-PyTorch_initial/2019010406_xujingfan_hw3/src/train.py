...
from lossplt import loss_plot

save_name = "epoch_loss"
    ...
    with torch.no_grad():
        ...
        for images, labels in test_loader:
            ...
            for i in range(len(predicted)):
                if not predicted[i] == labels[i]:
                    torchvision.utils.save_image(images[i],"results/"+"P"+ str(predicted[i].item())+"L"+str(labels[i].item())+".png")
...
def train(epochs, batch_size, learning_rate, num_classes):
    ...
    torch.save(loss0, save_name) # save loss
